...
logger = logger_gen('localization_pf')
...
class LocalizationModule:
    def __init__(self,
        ...
        self.vel_estimate = self.transition_model_max_vel = float(algo_params['max_vel'])
        ...
        self.floor = None
        logger.info(f"Initial localizationModule SUCCESS!")

    def estimate_position(self,
                          beacon_data_batch: Union[List[Dict], None],
                          imu_data_batch: Union[List[Dict], None],
                          wifi_data_batch: Union[List[Dict], None],
                          mpu_vel_q: -1) -> Dict:  # {'x', 'y', 'z', 'timestamp'}
        # todo: not able to handle 3d positioning
        
        if mpu_vel_q != -1:
            self.vel_estimate = mpu_vel_q
            if mpu_vel_q < 0.5:
                self.vel_estimate = 0.5
        else:
            self.vel_estimate = self.transition_model_max_vel
        # print("in estimate_position", self.vel_estimate,'m/s')
        ...
        if not self.particles:
            if self.cur_rssi_info:
                ...
                for beacon_data in self.cur_rssi_info:
                    if beacon_data['source']['source_identifier'] not in duplicated_beacon:
                        duplicated_beacon.append(beacon_data['source']['source_identifier'])
                        x, y = beacon_data['source']['source_pos']['x'], beacon_data['source']['source_pos']['y']
                        anchor_points_with_range.append(((x, y), 30))  # todo: radius is related to position type, will lead to error if the scale is not in meter
                logger.info(f"Setup receiver: {beacon_data_batch[0]['target_identifier']}.")
        ...
        # self.delta_t = 0
        return {'x': cur_x, 'y': cur_y, 'z': cur_z, 'timestamp': self.cur_ts, 'raw_p': self.particles}
    ...
    def _weight_update(self):
        ...
        try:
            ...
            # logger.error(f"Weight update: Zero Division Error.")
    ...
    def _do_resample(self):
        ...
        if self.div_by_zero:
            ...
            # logger.info(f"Resampleing: Solve Zero Division Error (random_particle_nums = {self.num_of_random_particles}).")
    ...
    def _propagate_particles(self):
        ...
        #     # logger.info(f"Propagate particles: not able to propagate: {delta_t * self.transition_model_max_vel}.")
        #     # return
        # else:
        # max_distance_in_cur_step = ((delta_t * self.transition_model_max_vel))
        max_distance_in_cur_step = ((delta_t * self.vel_estimate))
        ...
        while i < self.num_of_particles:
            ...
            offset_x = np.random.uniform(-max_distance_in_cur_step, max_distance_in_cur_step)
            offset_y = np.random.uniform(-max_distance_in_cur_step, max_distance_in_cur_step)


            tmp_x, tmp_y = pos[0] + offset_x, pos[1] + offset_y
            for polygon in self.map_constraints:
                if self._particle_within_range((tmp_x, tmp_y), polygon):
                    self.particles[i] = ((round(tmp_x,2), round(tmp_y,2)), self.particles[i][1])
            i += 1
        ...
        if not anchor_points_with_init_range:
            ...
            if self.map_constraints:
                for anchor_point, radius in anchor_points_with_init_range:
                    ...
                    # print(len(self.map_constraints), len(anchor_points_with_init_range), anchor_point)
                    for polygon in self.map_constraints:
                        # print(polygon)
                        res += self._2d_polygon_intersect_points(x_min=x_min,
                                                                 y_min=y_min,
