<section id="widget-grid" class="">
    ...
    <div class="row">
                ...
                <div>
                    ...
                    <div class="widget-body">
                        <div class="row no-space">
                            <div class="col-xs-12 col-sm-12 col-md-8 col-lg-12" style="height:460px;">
                                <nav id="menu"></nav>
                                <div id="map"></div>
                            </div>
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                                <!-- <div id="graph-C" class="chart no-padding"></div> -->
                            </div>
                            <div class="note">
                                <strong>NOTE: 
        ...
        <?php foreach ($sensor_data as $sensor_data_item) {?>
        ...
        <div class="jarviswidget jarviswidget-color-blueDark"
            ...
            <header>
                <span class="widget-icon"> <i class="fa fa-map-marker"></i> </span>
                <h2>Sensor-#<?=$sensor_data_item->label?>  (<?=$sensor_data_item->sensor?>)</h2>
            </header>
            ...
            <div>
                ...
                <div class="widget-body">
                    <div class="row no-space">
                        <div class="note">
                            <strong>BEACON TIMESTAMP 
                            </strong>
                        </div>
                        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                            <div id="graph-beacon-<?=$sensor_data_item->label?>" class="chart no-padding" style="height: 150px;"></div>
                        </div>
                        <div class="note">
                            <strong>TRAJECTORY TIMESTAMP 
                            </strong>
                        </div>
                        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                            <div id="graph-trajectory-<?=$sensor_data_item->label?>" class="chart no-padding" style="height: 150px;"></div>
                        </div>
                        <div class="note">
                            <strong>NOTE: 
                                <font color="#04B404">GREEN DOT -> go</font> |
...
<script type="text/javascript">
    ...
    var map = new mapboxgl.Map({
        ...
        zoom: <?= $site_info[0]['mapbox_zoom']?> - 1,
    ...
    function runMorrisCharts(){
        <?php foreach ($sensor_data as $key => $sensor_data_item) {?>
            console.log(<?= $key ?>)
        if ($('#graph-beacon-<?= $sensor_data_item->label ?>').length){ 
            var neg_data = [
                <?php
                    $start = $sensor_data_item->beacon_data[0]['ts'];
                    $pin = 0;
                    foreach ($sensor_data_item->beacon_data as $beacon_item) {
                        if ($beacon_item['ts'] - $start < 60) {
                            $pin = 1;
                        }else {
                            if ($pin) {
                                echo '{time :'.($start * 1000). ',vel: 1},';
                            }else {
                                echo '{time :'.($start * 1000). ',vel: 0},';
                            }
                            $pin = 0;
                            $start = $start + 60;
                        }
            ...
            Morris.Line({
                element: 'graph-beacon-<?= $sensor_data_item->label ?>',
                data: neg_data,
                xkey: 'time',
                ykeys: ['vel'],
        ...
        if ($('#graph-trajectory-<?= $sensor_data_item->label ?>').length){ 
            var neg_data = [
                <?php
                    $start = $sensor_data_item->beacon_data[0]['ts'];
                    $pin = 0;
                    foreach ($sensor_data_item->trajectory_data as $trajectory_item) {
                        if ($trajectory_item['ts'] - $start < 60) {
                            $pin = 1;
                        }else {
                            if ($pin) {
                                echo '{time :'.($start * 1000). ',vel: 1},';
            ...
            Morris.Line({
                element: 'graph-trajectory-<?= $sensor_data_item->label ?>',
                data: neg_data,
                xkey: 'time',
                ykeys: ['vel'],
                dateFormat: function (ts) {
