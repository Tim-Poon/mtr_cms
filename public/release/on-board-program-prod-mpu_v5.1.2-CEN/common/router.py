...
class Router:
                ...
                while not tmp_q.empty():
                    ...
                    for callback in self.__listeners[msg.event_type]:  # todo: may need a lock, because of self.register_listener()
                        try:
                            ...
                            self.dispatch_event(LogEvent(identifier=None,  # note: don't wanna couple with business
    ...
    def __self_examination(self):
        while 1:
            logger.info(f'Router Pool = {self.__msg_q.qsize()}')
            time.sleep(60)
