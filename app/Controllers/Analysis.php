...
class Analysis extends Controller
{
	...
	public function _remap($method, ...$params)
	{
		...
		{
			$analysis_date = $method;
			$site_name = $params[0];
			return $this->view_analysis($analysis_date, $site_name);
		}
	}
	...
	public function index()
	{
		...
		echo view('head', $data);
		echo view('js');
		echo view('ajax/analysis', $data);
		echo view('foot');
	}

	public function view_analysis($analysis_date, $site_name)
	{
		// if ($repord_id){
		...
		// 	$trajectory_data = [];
		// }
        $s_time = Time::createFromFormat('Ymd-His', $analysis_date.'-000000', 'Asia/Hong_Kong');
        $s_timestamp = $s_time->getTimestamp();
        $e_timestamp = $s_timestamp + 24 * 60 * 60;
        $site_info = $this->model_monitor->get_site_info_by_name($site_name);
        $site_sensors = $this->model_monitor->get_site_sensors($site_info[0]['site']);
        
        $sensor_data = [];
        foreach ($site_sensors as $key => $sensor) {
            $sensor->beacon_data = $this->model->get_sensor_raw_beacon_data($sensor->sensor, $s_timestamp, $e_timestamp);
            $sensor->trajectory_data = $this->model->get_sensor_raw_loc_data($sensor->sensor, $s_timestamp, $e_timestamp);
            array_push($sensor_data, $sensor);
            // print_r($sensor);
        }
        // $data_beacon = $this->model->get_sensor_raw_beacon_data($site_sensors[0]->sensor, $s_timestamp, $e_timestamp);
        $data_loc = $this->model->get_sensor_raw_loc_data($site_sensors[0]->sensor, $s_timestamp, $e_timestamp);
        // echo($site_sensors[0]->sensor.'<br> '.$site_sensors[0]->label. '<br>'. $s_timestamp.'<br>'. $e_timestamp.'<br>');
        // echo(count($data_beacon).'<br> ');
        // echo(count($data_beacon));
        // print_r($data);
		$data = 
		[
			'icon' => 'fa-bar-chart-o',
			'title' => 'Analysis',
			'sub_title' => ' > '.$analysis_date .' - '.$site_name,
			'site_names' => $this->model_monitor->get_site_name_all(),
			...
			'trajectory_data' => $data_loc,
            // 'beacon_data' => $data_beacon,
            'sensor_data' => $sensor_data,

            'site_sensors' => $site_sensors,
            //get site sensor -> get sensor raw -> get sensor loc -> get sensor log

		];
		echo view('head', $data);
		echo view('js');
		echo view('ajax/analysis_data', $data);
		echo view('foot');
	}

	public function get_analysis_full_data($delivery_date, $repord_id)
	{
		$report_by_id = $this->model->get_report_by_id($repord_id);
		$report_item = $report_by_id[0];
		$site_info = $this->model_monitor->get_site_info_by_name($report_item['site']);
