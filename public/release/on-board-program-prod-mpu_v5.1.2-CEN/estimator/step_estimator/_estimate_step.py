...
ACC_X = ACC_Y = ACC_Z = GYRO_X = GYRO_Y = GYRO_Z = MAG_X = MAG_Y = MAG_Z = TS = float
ACCELEROMETER_DATA = Tuple[TS, ACC_X, ACC_Y, ACC_Z]
...
# main func
# def compute_step_positions(acce_datas,
#                            ahrs_datas,
#                            posi_datas):
...
#
#     return step_positions  # [[ts, delta_x, delta_y]]

def estimate_num_of_step(acce_datas: List[ACCELEROMETER_DATA]) -> int:
    try:
        acce_datas = np.array(acce_datas)
        step_timestamps, step_indexs, step_acce_max_mins = compute_steps(acce_datas)
        # print(f'step_timestamps = {step_timestamps}, len = {len(step_timestamps)} \n'
        #       f'step_indexs = {step_indexs}, len = {len(step_timestamps)} \n'
        #       f'step_acce_max_mins = {step_acce_max_mins}, len = {len(step_acce_max_mins)} \n')

        # stride_lengths = compute_stride_length(step_acce_max_mins)
        # pprint.pprint(stride_lengths)
        # print(f'stride_lengths = {stride_lengths}, len = {len(stride_lengths)}')
    except:
        return 0
    else:
        return len(step_timestamps)
